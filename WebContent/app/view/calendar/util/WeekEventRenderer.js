 ...
 */
Ext.define('AboutUs.view.calendar.util.WeekEventRenderer', {

    requires: ['AboutUs.view.calendar.util.Date'],
        ...
        render: function(o) {
            ...
            for (; w < weekCount; w++) {
                if (!grid[w] || grid[w].length == 0) {
                    ...
                    dt = AboutUs.view.calendar.util.Date.add(dt, {days: 7});
                } else {
                    var row,
                        d = 0,
                        wk = grid[w],
                        startOfWeek = Ext.Date.clone(dt),
                        endOfWeek = AboutUs.view.calendar.util.Date.add(startOfWeek, {days: dayCount, millis: -1});
                        ...
                        if (wk[d]) {
                            ...
                            for (; ev < ct; ev++) {
                                ...
                                if (!evt.isSpan || evt.isSpanStart) {
                                    ...
                                    item._weekIndex = w;
                                    item._renderAsAllDay = item[AboutUs.view.calendar.data.EventMappings.IsAllDay.name] || evt.isSpanStart;
                                    item.spanLeft = item[AboutUs.view.calendar.data.EventMappings.StartDate.name].getTime() < startOfWeek.getTime();
                                    item.spanRight = item[AboutUs.view.calendar.data.EventMappings.EndDate.name].getTime() > endOfWeek.getTime();
                                    ...
                                    var diff = AboutUs.view.calendar.util.Date.diffDays(dt, item[AboutUs.view.calendar.data.EventMappings.EndDate.name]) + 1,
