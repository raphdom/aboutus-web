/**
 * @class AboutUs.view.calendar.view.AbstractCalendar
 ...
 */
Ext.define('AboutUs.view.calendar.view.AbstractCalendar', {
    extend: 'Ext.Component',
    alias: 'widget.calendarview',
    requires: [
        'AboutUs.view.calendar.util.Date',
        'AboutUs.view.calendar.data.EventMappings'
    ],
    /**
     * @cfg {Number} startDay
     ...
     * {@link AboutUs.view.calendar.EventRecord}) to populate the calendar views with events. Internally this method
    ...
    initComponent: function() {
        ...
        this.addEvents({
             ...
             * @param {AboutUs.view.calendar.view.AbstractCalendar} this 
             */
            eventsrendered: true,
            /**
             * @event eventclick
             ...
             * @param {AboutUs.view.calendar.view.AbstractCalendar} this
             * @param {AboutUs.view.calendar.EventRecord} rec The {@link AboutUs.view.calendar.EventRecord record} for the event that was clicked on
             ...
             * @param {AboutUs.view.calendar.view.AbstractCalendar} this
             * @param {AboutUs.view.calendar.EventRecord} rec The {@link AboutUs.view.calendar.EventRecord record} for the event that the cursor is over
             ...
             * @param {AboutUs.view.calendar.view.AbstractCalendar} this
             * @param {AboutUs.view.calendar.EventRecord} rec The {@link AboutUs.view.calendar.EventRecord record} for the event that the cursor exited
             ...
             * @param {AboutUs.view.calendar.view.AbstractCalendar} this
             * @param {AboutUs.view.calendar.EventRecord} rec The {@link AboutUs.view.calendar.EventRecord record} for the event that was moved with
             ...
             * @param {AboutUs.view.calendar.view.AbstractCalendar} this
             * @param {AboutUs.view.calendar.EventRecord} rec The {@link AboutUs.view.calendar.EventRecord record} for the event that was deleted
             */
            //eventdelete: true
        });
    },
    ...
    getWeekCount: function() {
        var days = AboutUs.view.calendar.util.Date.diffDays(this.viewStart, this.viewEnd);
            ...
            for (d = 0; d < this.dayCount; d++) {
                if (evtsInView.getCount() > 0) {
                    var evts = evtsInView.filterBy(function(rec) {
                        var startDt = Ext.Date.clearTime(rec.data[AboutUs.view.calendar.data.EventMappings.StartDate.name], true),
                            startsOnDate = dt.getTime() == startDt.getTime(),
                            spansFromPrevView = (w == 0 && d == 0 && (dt > rec.data[AboutUs.view.calendar.data.EventMappings.StartDate.name]));
                ...
                dt = AboutUs.view.calendar.util.Date.add(dt, {days: 1});
            }
        }
    ...
    prepareEventGrid: function(evts, w, d) {
        ...
        evts.each(function(evt) {
            var M = AboutUs.view.calendar.data.EventMappings,
            days = AboutUs.view.calendar.util.Date.diffDays(
            AboutUs.view.calendar.util.Date.max(me.viewStart, evt.data[M.StartDate.name]),
            AboutUs.view.calendar.util.Date.min(me.viewEnd, evt.data[M.EndDate.name])) + 1;

            if (days > 1 || AboutUs.view.calendar.util.Date.diffDays(evt.data[M.StartDate.name], evt.data[M.EndDate.name]) > 1) {
    ...
    onEventDrop: function(rec, dt) {
        if (AboutUs.view.calendar.util.Date.compare(rec.data[AboutUs.view.calendar.data.EventMappings.StartDate.name], dt) === 0) {
            // no changes
            return;
        }
        var diff = dt.getTime() - rec.data[AboutUs.view.calendar.data.EventMappings.StartDate.name].getTime();
        rec.set(AboutUs.view.calendar.data.EventMappings.StartDate.name, dt);
        rec.set(AboutUs.view.calendar.data.EventMappings.EndDate.name, AboutUs.view.calendar.util.Date.add(rec.data[AboutUs.view.calendar.data.EventMappings.EndDate.name], {millis: diff}));
    ...
    onCalendarEndDrag: function(start, end, onComplete) {
        if (start && end) {
            ...
            var o = {};
            o[AboutUs.view.calendar.data.EventMappings.StartDate.name] = start;
            o[AboutUs.view.calendar.data.EventMappings.EndDate.name] = end;
    ...
    onUpdate: function(ds, rec, operation) {
        ...
        if (operation == Ext.data.Record.COMMIT) {
            ...
            if (this.enableFx && this.enableUpdateFx) {
                this.doUpdateFx(this.getEventEls(rec.data[AboutUs.view.calendar.data.EventMappings.EventId.name]), {
                    scope: this
                });
            }
        }
    ...
    onAdd: function(ds, records, index) {
        ...
        if (this.enableFx && this.enableAddFx) {
            this.doAddFx(this.getEventEls(rec.data[AboutUs.view.calendar.data.EventMappings.EventId.name]), {
                scope: this
            });
        }
    },
    ...
    onRemove: function(ds, rec) {
        ...
        if (this.enableFx && this.enableRemoveFx) {
            this.doRemoveFx(this.getEventEls(rec.data[AboutUs.view.calendar.data.EventMappings.EventId.name]), {
                remove: true,
                scope: this,
            ...
            this.getEventEls(rec.data[AboutUs.view.calendar.data.EventMappings.EventId.name]).remove();
            this.refresh();
        }
    },
    ...
    isEventVisible: function(evt) {
        var M = AboutUs.view.calendar.data.EventMappings,
            ...
            evEnd = AboutUs.view.calendar.util.Date.add(data[M.EndDate.name], {seconds: -1}).getTime();
    ...
    isOverlapping: function(evt1, evt2) {
        ...
        M = AboutUs.view.calendar.data.EventMappings,
        start1 = ev1[M.StartDate.name].getTime(),
        end1 = AboutUs.view.calendar.util.Date.add(ev1[M.EndDate.name], {seconds: -1}).getTime(),
        start2 = ev2[M.StartDate.name].getTime(),
        end2 = AboutUs.view.calendar.util.Date.add(ev2[M.EndDate.name], {seconds: -1}).getTime();
    ...
    setViewBounds: function(startDate) {
        var start = startDate || this.startDate,
            ...
            Dt = AboutUs.view.calendar.util.Date;
     ...
     * of {@link #AboutUs.view.calendar.EventRecord EventRecord} objects
     */
    sortEventRecordsForDay: function(evts) {
        ...
        evts.sortBy(Ext.bind(function(evtA, evtB) {
            ...
            if (this.spansHavePriority) {
                ...
                var diff = AboutUs.view.calendar.util.Date.diffDays;
    ...
    moveNext: function(noRefresh) {
        return this.moveTo(AboutUs.view.calendar.util.Date.add(this.viewEnd, {days: 1}));
    },
    ...
    movePrev: function(noRefresh) {
        var days = AboutUs.view.calendar.util.Date.diffDays(this.viewStart, this.viewEnd) + 1;
    ...
    moveMonths: function(value, noRefresh) {
        return this.moveTo(AboutUs.view.calendar.util.Date.add(this.startDate, {months: value}), noRefresh);
    },
    ...
    moveWeeks: function(value, noRefresh) {
        return this.moveTo(AboutUs.view.calendar.util.Date.add(this.startDate, {days: value * 7}), noRefresh);
    },
    ...
    moveDays: function(value, noRefresh) {
        return this.moveTo(AboutUs.view.calendar.util.Date.add(this.startDate, {days: value}), noRefresh);
    },
    ...
    /**
     * Sets the event store used by the calendar to display {@link AboutUs.view.calendar.EventRecord events}.
    ...
    getEventRecord: function(id) {
        var idx = this.store.find(AboutUs.view.calendar.data.EventMappings.EventId.name, id);
